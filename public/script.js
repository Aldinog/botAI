...
const login = async () => {
    ...
    try {
        ...
        if (response.ok && data.success) {
            ...
            authStatus.innerHTML = `<span style="color: #ef4444;">${data.error || 'Authentication Failed'}</span>`;
            if (tg.HapticFeedback) tg.HapticFeedback.notificationOccurred('error');
        }
    } catch (err) {
        authStatus.innerHTML = '<span style="color: #ef4444;">Network Error. Try again later.</span>';
    }
};
